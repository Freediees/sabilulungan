    ...
    <div class="wrapper clearfix">        
        <div class="primary">
            <ul class="project-list-wrapper list-nostyle clearfix">
                ...
                $Qlist = $this->db->query("SELECT a.id, a.user, a.judul, a.latar_belakang, a.current_stat, b.name, c.name AS skpd, SUM(d.amount) AS mohon, e.name AS tahap
                                        FROM proposal a
                                        ...
                                        WHERE a.user_id='$id'
                                        GROUP BY a.id ORDER BY a.id DESC LIMIT $position,$limit");
            ...
            if($Qpaging->num_rows > $limit){
                $this->ifunction->paging($p, site_url('lpj').'/list/'.$id.'/', $num_page, $Qpaging->num_rows, 'href', false);
            }
            ?>
        </div>
        <!-- primary -->
...
<div role="main" class="content-main" style="margin:120px 0 50px">
    <div class="register-page wrapper-half">
        ...
        <form class="form-global" method="post" action="<?php echo base_url('process/lpj/add/'.$dx) ?>" enctype="multipart/form-data">
            <fieldset>
