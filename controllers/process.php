...
class Process extends CI_Controller {
	...
	public function tatausaha($tp, $dx=0)
	{
		...
		switch($tp){			
			...
			if($user_id && $role_id && $kategori && $kategori2 && $kelengkapan && $persyaratan && $keterangan){
				if($kategori2 == 1){
					if(count($kelengkapan) < 4 || count($persyaratan) < 21){
						$_SESSION['notify']['type'] = 'failed';
						$_SESSION['notify']['message'] = (count($kelengkapan));
					...
					if(count($kelengkapan) < 4 || count($persyaratan) < 17){
						$_SESSION['notify']['type'] = 'failed';
						$_SESSION['notify']['message'] = (count($kelengkapan));
						$_SESSION['notify']['message'] = 'Silahkan lengkapi semua persyaratan.';
	...
	{	
	//ferdi
	//echo "<script type='text/javascript'>alert('masuk tapd');</script>";
                    ...
                    if($Qlist->num_rows){
                        ...
                        foreach($Qlist->result_object() as $list){
                            ...
                            echo '<tr>
                                    ...
                                    <td colspan = "3">'; if(isset($besar[1]->value)){ echo 'Rp. '.number_format($besar[1]->value,0,",",".").',-'; $total_timbang += $besar[1]->value; }else echo '-'; echo '</td>
                                    <td>'; if(isset($besar[2]->value)) echo $besar[2]->value; else echo '-'; echo '</td>
                                </tr>';
                            $i++;
                        }
